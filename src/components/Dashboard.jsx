...
function PnlBadge({ value, percent }) {
  ...
  return (
    ...
    >
      {positive ? "+" : ""}€{value.toFixed(2)} ({positive ? "+" : ""}
      {percent.toFixed(2)}%)
    </span>
  );
}
...
function Dashboard() {
  ...
  return (
    ...
    >
      <div
        className="fixed top-0 left-0 right-0 bg-gray-950 z-50"
        style={{ height: "44px" }}
      />
              ...
              {portfolio.length > 0 && (
                <div className="bg-gray-900 border border-indigo-500/40 rounded-xl px-6 pt-5 pb-6 mb-4 text-center">
                  ...
                  <div className="mt-2 flex justify-center">
                    <span
                      className={`text-sm font-semibold px-3 py-1 rounded-full ${totalPnl >= 0 ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                    >
                      {totalPnl >= 0 ? "+" : ""}
              ...
              {portfolio.length > 0 ? (
                <div className="space-y-3">
                  {portfolio.map((etf) => {
                    ...
                    const pnl = currentValue
                      ? currentValue - etf.invested
                      : null;
